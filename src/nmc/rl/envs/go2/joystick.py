"""Joystick (velocity-command) task for Go2.

Direct port of mujoco_playground._src.locomotion.go1.joystick.Joystick, with
Go1Env/go1_constants swapped for Go2Env/go2_constants. All obs/reward logic
is generic quadruped code (relies only on qpos[7:]/qvel[6:] DOF slicing, per-
foot geoms/sites, and the imu-site sensors defined in base.py) -- no Go1-
specific assumptions beyond the constants module, so it carries over
unchanged. Reward scales are Go1's as an initial guess (Go1 and Go2 are
similar mass/size); revisit if training doesn't converge as well as Go1's.

Unlike Go1, there is no Kp/Kd config here -- Go2's XML actuators already bake
in tuned PD gains (see go2/base.py).
"""
...
from nmc.rl.envs.go2 import base as go2_base
from nmc.rl.envs.go2 import go2_constants as consts
