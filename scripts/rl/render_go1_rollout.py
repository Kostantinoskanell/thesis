"""Render the saved Go1 rollout to a GIF + make a velocity-tracking graph.

Uses Playground's exact Go1 mj_model (so qpos matches), replays the saved qpos
trajectory through an offscreen renderer. Runs in WSL2 (needs EGL: MUJOCO_GL=egl).
"""
...
def main():
    ...
    ap.add_argument("--rollout", default="/mnt/c/Users/hapos/Desktop/thesis/assets/go1_rollout.npz")
    ap.add_argument("--outdir", default="/mnt/c/Users/hapos/Desktop/thesis/archive/D2_go2_rl")
    args = ap.parse_args()
    ...
    a1.set_title("Go1 RL policy — velocity tracking + stability (MuJoCo, full dynamics)")
    a2.plot(t, heights, color="#1f9d3a", lw=1.6)
    a2.axhspan(0.26, 0.34, color="#2e7d32", alpha=0.12, label="upright band")
