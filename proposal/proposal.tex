...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \textbf{Robot:} Ground robot with 360° LiDAR (32 beams,
  ...
  \item \textbf{Environment:} Enclosed arena ($10 \times 10$\,m) with randomized static
        obstacles (boxes, cylinders) and dynamic obstacles moving at 0.2--0.5\,m/s,
        i.e.\ below the robot's forward speed so that evasion is always kinematically
        possible (a faster obstacle could catch the robot from behind, making some
        collisions unavoidable for \emph{any} controller and compressing the
        inter-controller differences the study measures).
...
\paragraph{Two-layer control and robot platform.}
The neural controller is the \emph{high-level navigator}: it maps sensor observations
to a body-velocity command $[v_x,\,v_y,\,\omega]$ (realized as the discrete actions
forward/left/right/brake). This interface is platform-agnostic and decouples the
navigation contribution from low-level actuation. The core comparative study runs on a
fast kinematic-unicycle model for statistical throughput. For realistic validation and
a hardware demonstration, the same velocity commands drive a \textbf{Unitree Go2}
quadruped: in simulation via a convex-MPC locomotion controller, and on the physical
robot via the onboard SDK's velocity-command (``sport'') mode---so no bespoke gait is
deployed to hardware. The locomotion layer is developed in parallel and does not affect
the plasticity results.
