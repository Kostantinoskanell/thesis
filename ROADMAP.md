...
| L3 | **Hybrid PPO in Isaac Lab** — PopSAN actor + **deep-MLP critic** (only the deployed policy must be spiking; critic stays standard, per PopSAN) | spiking Go2 walks, reward ≈ MLP baseline (36.25) within a few % | ☐ — inject via rsl_rl 5.x `class_name`→`resolve_callable` (qualified path, no source edits); actor predicts the action-distribution **mean**, rsl_rl adds the Gaussian std + clipped-surrogate PPO loss (exactly how PopSAN integrates PPO). Custom policy must satisfy rsl_rl's API (act/evaluate/output_std/as_jit/as_onnx). |
