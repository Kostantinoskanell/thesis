...
| L3 | **Hybrid PPO in Isaac Lab** — PopSAN actor + **deep-MLP critic** (only the deployed policy must be spiking; critic stays standard, per PopSAN) | spiking Go2 walks, reward ≈ MLP baseline (36.25) within a few % | ◐ **integration WORKING; full run underway.** `SpikingActorMLPModel` (`src/nmc/locomotion/rsl_rl_spiking.py`) subclasses `rsl_rl.models.MLPModel` and swaps `self.mlp` for the PopSAN net; injected via `cfg["actor"]["class_name"]` with **zero edits** to Isaac Lab / rsl_rl. Wiring = a generated `train_spiking.py` (`scripts/make_isaac_train_spiking.py` patches Isaac Lab's train.py: adds repo `src` to path + sets the actor class_name on `agent_cfg.to_dict()` before the runner). **Smoke PASSED:** Isaac Lab built `Actor Model: SpikingActorMLPModel(mlp: PopSpikingActorNet(...))` with a standard MLP critic + GaussianDistribution, trained 15 iters clean (TRAIN_EXIT=0). **Integration ✅ working; performance tuning OPEN.** Full run (2048 envs, 1500 iters) trained clean but **underperforms**: reward **0.68** vs the MLP baseline's **36.25** (vel-err 1.38 vs 0.16 m/s). Diagnosed the main cause — the Go2 cfg has `actor_obs_normalization=False`, so raw obs (joint vels, commands, magnitudes ≫ [−3,3]) hit the fixed Gaussian receptive fields → most input dims give ~0 stimulation → lost information (an MLP tolerates unnormalized obs; a population encoder can't). Fix = inject `obs_normalization=True` (MLPModel normalizes *before* the spiking encoder) + T 5→8: **reward 0.68→2.9 by iter 400** (~4× better, reached 4× faster) — real improvement, still far from baseline. **Remaining tuning levers (each = one 20–75 min run):** T→10–16; firing sparsity (init ~35% is dense — tune `v_th`/`weight_gain` toward ~10–15%); larger `out_pop` + check decoder output scale vs the joint-target range; lower PPO lr (1e-3 is MLP-tuned); more iters. PopSAN/legged papers tuned extensively — this is expected spiking-RL tuning, not a pipeline flaw. (`scripts/wsl_isaac_go2_spiking.sh`, `scripts/make_isaac_train_spiking.py`) |
