...
| L3 | **Hybrid PPO in Isaac Lab** — PopSAN actor + **deep-MLP critic** (only the deployed policy must be spiking; critic stays standard, per PopSAN) | spiking Go2 walks, reward ≈ MLP baseline (36.25) within a few % | ◐ **actor integration coded, wiring+run remains.** Injection point pinned: rsl_rl 5.x `PPO.construct_algorithm` does `actor_class = resolve_callable(cfg["actor"]["class_name"]); actor = actor_class(obs, obs_groups, "actor", num_actions, **cfg["actor"])`. The actor base `rsl_rl.models.MLPModel` already owns obs-group selection, normalization, the Gaussian distribution, log-prob/KL, and JIT/ONNX — so **`SpikingActorMLPModel`** (`src/nmc/locomotion/rsl_rl_spiking.py`) subclasses it and just swaps `self.mlp` for the PopSAN net (verified design against the read MLPModel source). **Remaining:** IsaacLab 2.3.2 loads cfg via **hydra** + `handle_deprecated_rsl_rl_cfg` (converts old `policy=RslRlPpoActorCriticCfg` → split `actor`/`critic`); set `actor.class_name = "nmc.locomotion.rsl_rl_spiking:SpikingActorMLPModel"` (+ spiking kwargs) after that conversion (custom train script or hydra override), then train & compare to the MLP baseline. |
