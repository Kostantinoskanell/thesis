...
| L3 | **Hybrid PPO in Isaac Lab** — PopSAN actor + **deep-MLP critic** (only the deployed policy is spiking, per PopSAN) | spiking Go2 walks, reward ≈ MLP baseline (36.25) within a few % | ✅ **integration done; performance CLOSED at a documented, verified ceiling (D12).** `SpikingActorMLPModel` (`src/nmc/locomotion/rsl_rl_spiking.py`) subclasses `rsl_rl.models.MLPModel`, swaps `self.mlp` for the PopSAN net, injected via `cfg["actor"]["class_name"]` — zero edits to Isaac Lab/rsl_rl. **Root cause of the initial ~2.5-reward flatline found:** PPO's adaptive KL-based LR schedule was crushing the spiking actor's effective LR (population-coded firing-rate outputs → noisier KL estimates than a smooth MLP). **Fix:** `schedule="fixed"`, `lr=1e-3` (== the MLP's own tuned value) — a real, reproducible **~3x gain**. **Verified at 2x the MLP's training budget** (3000 vs 1500 iters, overnight run): four measurement windows spanning iter 600-3000 are statistically indistinguishable (means 7.68-8.10, stdev 0.55-0.62) — **reward ≈8 is a genuine architectural ceiling, not a training-length issue** (vel-err 1.45 m/s vs. the MLP's 0.16). Gap vs. MLP (8 vs 36) likely reflects PopSAN's validated scale (small classic-control Gym tasks, obs≤111/act≤8) vs. Go2's much harder regime (48 obs, 12 continuous joints, contact-rich). **⚠ CORRECTED 2026-07-22 (`archive/L4_gait_check/`): reward ~8 is a NON-WALKING crouch local optimum, not a capacity ceiling.** Dumped + rendered a real rollout: the policy belly-flops (base 0.40→0.11 m), travels 0.49 m in 20 s, never tracks the velocity command — the ~1.4 m/s vel-err across every variant was the tell. It's an optimization/exploration failure (MLP walks under the same reward), so next levers are reward-shaping/exploration (penalize low height, NoisySAN, curriculum), NOT bigger nets. Lesson: watch the robot, don't just trust the scalar. (`scripts/wsl_isaac_go2_spiking.sh`, `scripts/make_isaac_train_spiking.py`, `src/nmc/locomotion/popsan_actor.py`) |
| L4 | Release the spiking policy's plastic weights to R-STDP, test terrain recovery (ice) | R-STDP gait adaptation recovers on ice where nav-layer R-STDP couldn't (M4c) | ✅ **done — walking spiking Go2 achieved (distillation+DAgger); R-STDP does NOT recover locomotion (destabilizes) — a meaningful nav-vs-loco contrast (D13).** Built entirely inside Isaac Lab/PhysX (not the Windows MuJoCo loop — L1-L3 never left it either, avoiding cross-simulator joint-order/asset risk): `PopSpikingRSTDPController` (`src/nmc/locomotion/popsan_rstdp.py`, mirrors the nav layer's D8/D9 design) + `scripts/l4_rstdp_terrain.py` (baseline/frozen-shift/rstdp-shift/retention, `Isaac-Velocity-Flat-Unitree-Go2-Play-v0`). Icy shift found via headroom screening (friction=0.05/0.15 too harsh — catastrophic instant falls; **friction=0.4 gives a real, bimodal 25% fall rate**). **Result: R-STDP with the nav layer's exact hyperparameters (eta=0.05, input+readout, TD-error, anchor=0.005) makes things WORSE under the shift** (fall rate 25%→47%→53% over the block, trending down not up) **but does NOT catastrophically forget the base task** (retention ≈77% of baseline, 0% fall rate — anchor holds). Mirrors the nav layer's own M4 history (first attempt also stalled at 7% before D8/D9 tuning reached 30%) — likely needs its own tuning pass (smaller eta first, given population coding's demonstrated fragility to perturbation in L3; then fewer plastic layers, stronger anchor), not a dead end. **⚠ SUPERSEDED 2026-07-22: L4 is ill-posed on this base policy** — `archive/L4_gait_check/` shows the L3 spiking policy doesn't walk (belly-flops into a stationary crouch), so "recover a gait under a shift" has no gait to recover; the R-STDP negative results here are uninformative. Pipeline sound + reusable. **✅ RESOLVED 2026-07-22 (autonomous session, `archive/L4_gait_check/`): a WALKING spiking Go2 policy achieved via distillation.** Reward-shaping alone failed (anti-crouch fixed the belly-flop 0.11→0.24m but the policy then found a stand-still optimum; won't discover walking via PPO). Distilled the walking MLP teacher instead (M3 recipe): 128k pairs → BC-train PopSAN actor (val-MSE 0.025) → **distilled spiking policy walks: 5/5 episodes, 0 falls, return 26-41 (matches the MLP), upright stepping gait verified by MuJoCo-replay video (`distilled_walk.gif`)** — first spiking Go2 locomotion policy found in the lit search. Limitation: falls under a *sustained constant* command (~5s) — covariate shift, fixable via DAgger (future). This walking policy is now a valid substrate for the actual R-STDP terrain-recovery experiment. **DAgger then made it robust to sustained commands** (was falling ~5s into a held command): student-driven rollouts under held cmds + teacher labels (153.6k pairs) -> retrain -> under sustained vx=0.5, 3/3 x 1000 steps, 0 falls, vx err 0.028 (matches MLP), ~9.3m forward (`dagger_walk_forward.gif`). Robust walking spiking Go2 achieved. **Then the actual R-STDP terrain-recovery experiment ran on it (`archive/L4_gait_check/rstdp_terrain_result.md`): R-STDP does NOT recover locomotion on ice — it DESTABILIZES the gait.** Frozen walker is ice-robust (0/6 falls at friction 0.06, tracks vx); releasing to R-STDP (eta 0.01 and 0.003, readout-only, anchored) progressively collapses it (eps 3-6 fall at len=85). Meaningful contrast: R-STDP works on the *navigation* layer (M4, discrete/coarse, sensor-remap) but fails on *locomotion* (continuous/delicate motor control — Hebbian perturbation breaks the gait faster than it adapts). Delineates where reward-modulated plasticity helps vs doesn't; motivates e-prop (D1) for fine motor control. |
| L5 | Energy of a spiking *locomotion* controller (H2) | SynOps + µJ/decision vs the MLP actor (Zhao et al. method, [M6](docs/references/snn_learning.md)); cite PopSAN's 140× Loihi figure as the neuromorphic-hardware ceiling | ✅ **done ([D15](docs/references/sota_decisions.md), `archive/L5_energy/`).** SynOps vs MACs on real walking obs, 45 nm (Horowitz 2014), `scripts/l5_energy_analysis.py`; walking re-verified by trajectory for every variant. **The naive distilled walker fires DENSELY (62.7% mean hidden, vs the nav ALIF-SNN's 1.4%) → 328.9 nJ = 0.57× the MLP (COSTLIER).** BC only matches actions, so nothing drives sparsity; population coding (obs 48→480) + T=8 multiply the cost. **Recovered H2 with two levers:** (a) a differentiable firing-rate penalty added to distillation (`PopSpikingActorNet.forward_with_activity` + `l4_distill_spiking.py --firing-penalty`) halves firing to 44% at negligible gait cost, but reaches only 0.70× (still costlier); (b) **T is the dominant energy knob but trades against gait fidelity** — T=4 is 1.36× cheaper yet **breaks walking** (belly-flops, path 2.6 m), while **T=5 is the minimum T that preserves the gait → 178.1 nJ = 1.04× cheaper than the MLP AND still walks** (base 0.190 m, vx 0.476, path 18.3 m; `sparse_t5_walk.gif`). **First walking spiking Go2 locomotion policy that is also (marginally) more energy-efficient than its MLP equivalent at 45 nm digital.** Honest caveat: the 1.04× margin is thin — locomotion does NOT get navigation's order-of-magnitude win (continuous motor control resists sparse firing), echoing L4/D13; and 45 nm digital SynOps is conservative (Loihi's event-driven AC gives PopSAN ~140× vs GPU — this is a floor, not the ceiling). **⚠ CORRECTED 2026-07-23 (user: "it walks too crouched i dont like it"):** the 0.19 m walker was faithful to its *teacher*, which itself only stood at 0.184 m (natural Go2 height ≈0.30 m) — the flat-velocity reward never demanded taller. Fixed at the source: retrained the MLP teacher with a base-height reward (target 0.30 m; no PPO-discovery problem for a plain MLP, unlike D14's spiking stand-still trap) — verified by trajectory, 0.279–0.305 m, 0 falls. Re-ran the whole collect→distill→DAgger→sparsify→T-sweep pipeline on the new teacher: **energy numbers hold — T=5 is still 1.04× cheaper (178.5 nJ) at the CORRECT standing height** (0.302 m, vx 0.507, 3/3×1000 steps 0 falls, confirmed by eye in `sparse_t5_upright_walk.gif`). One tuning wrinkle: the first T=5-upright retrain was fragile (2/3 episodes eventually fell despite good offline val-MSE) — a gentler retrain fixed it to 3/3 robust, another instance of offline-loss-≠-closed-loop-robustness. **⚠ SECOND CORRECTION 2026-07-23 (user, watching the video: "the 4th leg on behind is almost just dragging"):** quantified per-leg swing amplitude/jitter (`scripts/l4_leg_amplitude.py`) — confirmed the MLP teacher itself had the asymmetry (RR hip swing 0.123 vs 0.25–0.32 for the other legs), same root-cause pattern as the crouch. Fixed via Isaac Lab's `feet_slide` reward term (penalizes foot velocity during ground contact); weight −1.0 was a clean failure (policy froze rigid to dodge the penalty, all joint amplitudes → ~0.01, vx→0) — corrected to −0.1 (Isaac Lab's own humanoid-config value). Re-ran the whole pipeline: **energy holds — T=5 still ~1.03× cheaper (181.0 nJ)** at an even taller, less-asymmetric gait (0.338 m, vx 0.523, 3/3×1000 steps 0 falls, path 20.5 m — best metrics of any checkpoint yet). RR's whole-leg drag is resolved (hip amplitude went from smallest-of-four to largest); its thigh joint alone keeps a partial residual asymmetry — an honest, not-quite-perfect fix. Full record: `archive/L5_energy/README.md`, `archive/L4_gait_check/README.md` "DRAGGING REAR LEG FIX". |
...
### ▶ Continuation plan (decided 2026-07-22, end of the autonomous L-track push)

The L-track has now delivered its full scientific payload, and it converges on one clean,
defensible thesis message. **Decision: declare the L-track complete (L0–L5 done; L6
deferred as optional) and return to the main M-track for the rigorous, defense-grade
results.** Rationale + priority order:

1. **The L-track's contribution is a *layer-specific* story, and it's consistent across
   both hypotheses.** On the **navigation** layer, R-STDP recovers from shifts (M4: 7→30%)
   and the ALIF-SNN fires sparsely (1.4% → clearly energy-efficient). On the **locomotion**
   layer, the same R-STDP rule *destabilizes* the gait (L4/D13) and the distilled policy
   fires densely, so its energy win is thin (L5/D15: 1.04× only after sparsity reg + minimal
   T). **Conclusion for the write-up: neuromorphic online learning + sparse-coding
   efficiency are a natural fit for coarse, discrete navigation but a marginal, delicate fit
   for continuous fine motor control.** This is a more honest and interesting result than
   "SNNs win everywhere," and it motivates the D1 upgrade path (e-prop / per-neuron learning
   signals) for the locomotion layer as clearly-scoped future work rather than a gap.

2. **L6 (fully-spiking decode: ILC-SAN / MDC-SAN) is DEFERRED, not dropped.** It would only
   be worth it to push the locomotion policy's *reward* toward the MLP or to reclaim more
   energy — but distillation already gives a walking, energy-positive policy, so L6 is a
   diminishing-returns architecture swap. Revisit only if writing reveals the locomotion
   energy margin needs to be more than marginal.

3. **Highest-value remaining science = M5 + M6 on the nav layer** (the thesis's core
   quantitative table). M4/M4c are pilots (n=15–30, CIs overlap); M5 is the ≥10-seed,
   Holm-corrected, all-6-controllers × all-metrics comparison that the defense actually
   rests on, and M6 is its energy column (now with the L5 SynOps method proven and reusable).
   These turn "GO signal" into publishable numbers. **Do these next.**

4. **M4b extensions: cherry-pick, don't do all 7.** Prioritize (5) the D8/D9 ablation grid
   and (1) the dropout-severity sweep — they directly harden the M4 recovery claim. The rest
   (TM-NORM, firing-rate histograms, multi-mask) are nice-to-have figures; fold in only if
   time allows after M5/M6.

5. **FPGA track (M7–M8) is the hardware back half** — start once M5/M6 lock the simulation
   science, consistent with the "software front-loaded" plan. The R-STDP datapath golden
   reference (M0) and the SynOps/energy model (L5/M6) are the two things it needs upstream,
   and both now exist.

6. **Writing (M9) can start now** for everything already closed (M1–M4c, D-track, L0–L5) —
   the archive READMEs + decision log are structured to become thesis sections directly.

**Net: next concrete step is M5** (full comparison, ≥10 seeds, CIs) on the nav-layer
controllers, then M6 energy, then begin the FPGA datapath.
