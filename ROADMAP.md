...
| L3 | **Hybrid PPO in Isaac Lab** — PopSAN actor + **deep-MLP critic** (only the deployed policy must be spiking; critic stays standard, per PopSAN) | spiking Go2 walks, reward ≈ MLP baseline (36.25) within a few % | ◐ **integration WORKING; full run underway.** `SpikingActorMLPModel` (`src/nmc/locomotion/rsl_rl_spiking.py`) subclasses `rsl_rl.models.MLPModel` and swaps `self.mlp` for the PopSAN net; injected via `cfg["actor"]["class_name"]` with **zero edits** to Isaac Lab / rsl_rl. Wiring = a generated `train_spiking.py` (`scripts/make_isaac_train_spiking.py` patches Isaac Lab's train.py: adds repo `src` to path + sets the actor class_name on `agent_cfg.to_dict()` before the runner). **Smoke PASSED:** Isaac Lab built `Actor Model: SpikingActorMLPModel(mlp: PopSpikingActorNet(...))` with a standard MLP critic + GaussianDistribution, trained 15 iters clean (TRAIN_EXIT=0). **Integration ✅ working; performance tuning OPEN — 5 reference-grounded fixes tried, none broke the ceiling (D12).** Diffed the actor line-by-line against the real PopSAN PPO source (Tang et al. 2020 code). Tried, each isolated in a 400-iter run (vs. the MLP's 33.45 at the same checkpoint): `obs_normalization=True`+T=8 (→2.9, best), + reference σ=0.387 + actor-grad×0.1 emulating the paper's 10x actor/critic LR gap (→2.1-2.3), + rectangular surrogate replacing a fast-sigmoid shown 36x weaker at |v-thr|=0.2 (→1.9), removing the LR cut (→1.8-2.7), + tanh-bounded decoder matching the reference's "Squashed Gaussian" decoder (→0.82, worse). **None improved on the first fix; some were worse.** Rules out σ/actor-LR/surrogate-shape/decoder-bounding as *the* dominant cause. Attempted the full-1500-iter follow-up (best config); hit a practical wall — measured ~59s/iter (not the ~3s/iter the 400-iter runs suggested; GPU healthy, 54°C, no contention — likely this session's rough elapsed-time sampling underestimated true cost throughout), 1500 iters would be ~24h, killed rather than block. **Needs a fresh session with a properly timed iteration-rate check before the next long run.** (`scripts/wsl_isaac_go2_spiking.sh`, `scripts/make_isaac_train_spiking.py`, `src/nmc/locomotion/popsan_actor.py`) |
| L4 | Release the spiking policy's plastic weights to R-STDP, test terrain recovery (ice) | R-STDP gait adaptation recovers on ice where nav-layer R-STDP couldn't (M4c) | ☐ — the population/hidden weights are the plasticity sites; deploy-side numpy STDPLearner, same three-factor rule as the nav layer. |
